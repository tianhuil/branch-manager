...
dbCommand
    ...
    .option("--db-password <password>", "Database password (overrides DB_PASSWORD env var)")
    .option("--root-database-url <url>", "Root database URL (overrides ROOT_DATABASE_URL env var)")
    .action(async (options) => {
    try {
        const dbName = validateOption(options.dbName, process.env.DB_NAME, "Missing required parameter: dbName (provide via --db-name or DB_NAME env var)");
        const dbUser = validateOption(options.dbUser, process.env.DB_USER, "Missing required parameter: dbUser (provide via --db-user or DB_USER env var)");
        const dbPassword = validateOption(options.dbPassword, process.env.DB_PASSWORD, "Missing required parameter: dbPassword (provide via --db-password or DB_PASSWORD env var)");
        const rootDatabaseUrl = validateOption(options.rootDatabaseUrl, process.env.ROOT_DATABASE_URL, "Missing required parameter: rootDatabaseUrl (provide via --root-database-url or ROOT_DATABASE_URL env var)");
...
dbCommand
    ...
    .option("--db-name <name>", "Database name (overrides DB_NAME env var)")
    .option("--db-user <user>", "Database user to delete (overrides DB_USER env var, optional)")
    .option("--root-database-url <url>", "Root database URL (overrides ROOT_DATABASE_URL env var)")
    .action(async (options) => {
    try {
        const dbName = validateOption(options.dbName, process.env.DB_NAME, "Missing required parameter: dbName (provide via --db-name or DB_NAME env var)");
...
dbCommand
    ...
    try {
        ...
        const dbUser = validateOption(options.dbUser, process.env.DB_USER, "Missing required parameter: dbUser (provide via --db-user or DB_USER env var)");
        const dbPassword = validateOption(options.dbPassword, process.env.DB_PASSWORD, "Missing required parameter: dbPassword (provide via --db-password or DB_PASSWORD env var)");
        const dbHost = validateOption(options.dbHost, process.env.DB_HOST, "Missing required parameter: dbHost (provide via --db-host or DB_HOST env var)");
...
previewCommand
    ...
    .option("--branch-name <name>", "Branch name (overrides BRANCH_NAME env var, defaults to current git branch)")
    .option("--db-password-seed <seed>", "Password seed (overrides DB_PASSWORD_SEED env var)")
    .option("--root-database-url <url>", "Root database URL (overrides ROOT_DATABASE_URL env var)")
    .action(async (options) => {
    try {
        ...
        const branchName = validateOption(options.branchName || process.env.BRANCH_NAME, getCurrentGitBranch(), "Missing branch name (provide via --branch-name option, BRANCH_NAME env var, or run in a git repository)");
...
previewCommand
    ...
    .requiredOption("--branch-name <name>", "Branch name (overrides BRANCH_NAME env var, defaults to current git branch)")
    .option("--db-password-seed <seed>", "Password seed (overrides DB_PASSWORD_SEED env var)")
    .option("--db-host <host>", "Database host (overrides DB_HOST env var)")
    .action((options) => {
    try {
...
const validateOption = (option, processEnv, error) => {
